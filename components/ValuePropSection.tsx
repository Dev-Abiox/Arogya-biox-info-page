    ...
    <div className="relative z-20 py-6 md:py-24 px-6 md:px-12 lg:px-24">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 md:mb-24">
          ...
          <h2 className="text-2xl md:text-h2 lg:text-h1 font-light font-heading">Impact Across <span className="font-medium italic text-blue-300">Ecosystems</span></h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {values.map((v, i) => (
            <div key={i} className="glass-effect p-6 md:p-12 rounded-[40px] border border-white/10 md:hover:border-blue-400/30 md:hover:-translate-y-2 transition-all duration-500 flex flex-col group">
              ...
              <ul className="space-y-6 flex-grow">
                {v.benefits.map((benefit, j) => (
                  <li key={j} className="flex items-start gap-4 group/item">
                    ...
                    <span className="text-sm md:text-body-1 text-white/75 font-light group-hover:text-white/95 transition-colors leading-snug font-body">{benefit}</span>
                  </li>
                ))}
              </ul>
            </div>
